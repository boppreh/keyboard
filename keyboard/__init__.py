    ...
    def direct_callback(self, event):
        ...
        with _pressed_events_lock:
            if event_type == KEY_DOWN:
                _physically_pressed_keys[scan_code] = event
            keys_tuple = tuple(sorted(_physically_pressed_keys))
            keys_set = set(keys_tuple)
            if event_type == KEY_UP and scan_code in _physically_pressed_keys:
                del _physically_pressed_keys[scan_code]

        test_all_callbacks = lambda callbacks: all([c(event) for c in callbacks])

        if not test_all_callbacks(self.blocking_hooks) or not test_all_callbacks(self.blocking_keys[event.scan_code]):
            return False

        active_modifiers = tuple(key for key in keys_tuple if is_modifier(key))
        hotkey_callbacks = self.blocking_hotkeys[active_modifiers][keys_tuple]
        if hotkey_callbacks:
            _suppressed_keys.update(key for key in keys_set if not is_modifier(key))
            if not test_all_callbacks(hotkey_callbacks):
                return False
        
        if event_type == KEY_UP or not any(c and set(h).issuperset(keys_set) for h, c in self.blocking_hotkeys[active_modifiers].items()):
            # TODO: emit events in order they were received
            for key in (keys_set & _pending_keys) - _suppressed_keys:
                if key == scan_code and event_type == KEY_DOWN: continue
                press(key)
            _pending_keys.clear()
            accept = True
        else:
            _pending_keys.add(scan_code)
            accept = False

        if event_type == KEY_UP:
            _suppressed_keys.discard(scan_code)
            _pending_keys.discard(scan_code)
...
def _add_hotkey_step(handler, combinations, suppress):
    ...
    if suppress:
        ...
        for scan_codes in combinations:
            modifiers = tuple(sorted(scan_code for scan_code in scan_codes if is_modifier(scan_code)))
