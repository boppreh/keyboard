...
def build_tables():
    ...
    try:
        ...
        if e.returncode == 1:
            raise ValueError(
                'Failed to run dumpkeys to get key names. Check if your user is part of the "tty" group, and if not, add it with "sudo usermod -a -G tty USERNAME".'
            )
        else:
            raise
...
class Listener(object):
    ...
    def listen(self, callback):
        ...
        while self.is_running:
            ...
            char = names[0] if len(names[0]) == 1 else ""
            name = names[0]
