...
VK_PACKET = 0xE7
# An officially unassigned virtual key code, sent by send_menu_mask_key to
# stop the OS from reacting to modifier taps. Ignored by our own listener.
VK_MENU_MASK = 0xE8
    ...
    def listen(self, callback):
        ...
        def low_level_keyboard_handler(nCode, wParam, lParam):
            try:
                ...
                # Ignore events generated by SendInput with Unicode, and our
                # own menu mask key events.
                if vk not in (VK_PACKET, VK_MENU_MASK) and lParam.contents.flags & fake_alt != fake_alt:
...
def send_menu_mask_key():
    """
    Sends a no-op key event with an officially unassigned virtual key code.
    Used when suppressing another key while the user holds windows or alt:
    without this, the OS would see the modifier pressed and released with
    nothing in between and open the start menu (or focus the menu bar).
    Same technique as AutoHotkey's #MenuMaskKey.
    """
    user32.keybd_event(VK_MENU_MASK, 0, KEYEVENTF_KEYDOWN, 0)
